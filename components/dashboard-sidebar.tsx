...
import { LogOut, Package, History, UserCircle, Settings, Users } from "lucide-react";
...
export default function DashboardSidebar({ isAdmin }: DashboardSidebarProps) {
  ...
  const getLinkClass = (path: string) => {
    ...
    const base = "flex items-center gap-3 px-3 py-2.5 rounded-xl text-xs transition-all cursor-pointer";
    if (isActive) {
  ...
  return (
    <aside className="w-64 bg-dark-card border-r border-gray-900 flex flex-col justify-between p-6 shrink-0 z-10 shadow-[4px_0_24px_rgba(0,0,0,0.15)]">
      <div>
        {/* Cabecera / Logo */}
        <Link href="/dashboard" className="flex items-center gap-3 mb-8 pl-1 group">
        ...
        <nav className="space-y-1.5">
          ...
          <a
            href="#ajustes"
            className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-xs font-medium text-gray-500 hover:text-gray-400 border border-transparent transition-all pointer-events-none cursor-not-allowed select-none"
          >
