...
import { Shield, Sparkles, KeyRound, Mail, AlertTriangle, Loader2 } from "lucide-react";
...
export default function LoginPage() {
  ...
  useEffect(() => {
    const handleAuthTokensAndErrors = async () => {
      ...
      if (hash) {
        ...
        if (accessToken && refreshToken) {
          ...
          try {
            ...
            // Limpiar los parámetros de la URL para mayor seguridad
            window.history.replaceState(null, "", window.location.pathname);
      ...
      // Si no hay hash token válido pero hay un mensaje de error en los query params
      if (urlError) {
        setError(urlError);
      }
    };
  ...
  return (
    <div className="min-h-screen bg-dark-bg flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="absolute top-1/2 left-1/2 -z-10 h-[550px] w-[550px] -translate-x-1/2 -translate-y-1/2 rounded-full bg-gold-500/10 blur-[140px] pointer-events-none" />
      ...
      <div className="w-full max-w-md bg-dark-card border border-gold-500/20 rounded-3xl p-8 shadow-[0_20px_50px_rgba(0,0,0,0.5)] relative overflow-hidden backdrop-blur-md">
        ...
        {validatingInvite ? (
          <div className="flex flex-col items-center justify-center py-10 space-y-3">
            ...
            <span className="text-xs text-gray-300 font-medium">Validando enlace de invitación...</span>
            <p className="text-[11px] text-gray-500 text-center">
              Estamos verificando tus credenciales para redirigirte.
            </p>
          </div>
        ) : (
          <>
